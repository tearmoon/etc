...
description() {
  sed -Ei "s/^description=(\[.*][[:space:]]*)?/description=[ ⏲ $CURRENT_TIME | ϟ mode: $MODE !!!  ] /g" "$PROPFILE"
}

# my_device
if [ "${ETC}" = "disabled" ];then
  log "service etc disable"
  sed -Ei "s/^description=(\[.*][[:space:]]*)?/description=[ ⏲ $CURRENT_TIME | ϟ ETC: service $ETC !!!  ] /g" "$PROPFILE"
else
  optimization
  description
fi
